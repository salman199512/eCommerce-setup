...
class {{ $config->modelNames->name }}Controller extends AppBaseController
{
    ...
    public function store(CreateRequest $request) {
        ...
        session()->flash('message', '{{ $config->modelNames->name }} has been created successfully!');
        return Response::json(['message' => '{{ $config->modelNames->name }} has been created successfully.', 'back_url' => route('{{ $config->prefixes->getRoutePrefixWith('.') }}{{ $config->modelNames->camelPlural }}.index')]);
    }
    ...
    public function update({{ $config->modelNames->name }} ${{ $config->modelNames->camel }}, UpdateRequest $request) {
        ...
        session()->flash('alert-type', 'success');
        session()->flash('message', '{{ $config->modelNames->name }} has been updated successfully!');
        return Response::json(['message' => '{{ $config->modelNames->name }} updated successfully.', 'back_url' => route('{{ $config->prefixes->getRoutePrefixWith('.') }}{{ $config->modelNames->camelPlural }}.index')]);
    }
