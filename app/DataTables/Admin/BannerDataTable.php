    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ->editColumn('status', function (Banner $model) {
                ...
                $urlStatus = route('admin.banners.status-change', $model->uuid) . '?active=' . $val;

                return view('admin.layouts.status', compact('model', 'tableId','status','alert','urlStatus'));
            })
            ->editColumn('created_at', function (Banner $model){
                return GeneralHelperFunctions::prepareHtmlDate($model->created_at);
            })
            ->addColumn('action', 'admin.banners.datatables_actions')
